// ----- RANGES DEFINITION -----
// RFI (Raise First In) ranges for each position
const RANGES = {
    UTG: {
    ...
    BB: {
        // BB is special - not really RFI. We'll treat as "defend" mode or skip
        // For RFI training, BB rarely open-raises. We'll include tight range.
...
// Helper: Convert hand to range key (e.g., "AK" sorted)
function handToKey(hand) {
    ...
    // Sort so higher rank first (except for pairs)
...
let currentCorrectAction = null; // 'play' or 'fold'
let streak = 0;
...
function loadNewHand() {
    ...
    if (isMixedMode) {
        ...
        // Keep currentPosition (set by position buttons)
    }
    ...
    // Update UI
    renderCards(currentHand, [], 'handCards', null); // No board
    document.getElementById('currentPosDisplay').innerHTML = `📍 Position: ${currentPosition}`;
    document.getElementById('questionText').innerHTML = 'Play or Fold?';
    ...
    // Highlight current position in reference panel
...
function checkAnswer(action) {
    if (waitingForNext) {
        document.getElementById('feedbackArea').innerHTML = '⚠️ Click NEXT HAND for a new hand';
        return;
    }
    ...
    if (isCorrect) {
        ...
        waitingForNext = true; // Lock until NEXT is clicked
    }
}

// Override renderCards for preflop (no board)
